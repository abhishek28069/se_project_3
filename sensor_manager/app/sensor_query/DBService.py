...
url = "https://iaonqhgjxdkurmjpruoh.supabase.co"
key = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Imlhb25xaGdqeGRrdXJtanBydW9oIiwicm9sZSI6ImFub24iLCJpYXQiOjE2ODI3Mzc5OTgsImV4cCI6MTk5ODMxMzk5OH0.WahftY16sDHaOenx_WekCxmYAcItYqW8DL1m2d0Dvcs"
supabase = create_client(url, key)


def getGroups():
    response = (
        supabase.table("sensor_registry").select("group_name", count="exact").execute()
    )
...
def getTypes():
    response = (
        supabase.table("sensor_registry").select("sensor_type", count="exact").execute()
    )
    data = dict(response)["data"]
...
def getSensors(activated):
    ...
    if activated is not None:
        response = (
            ...
            .select("*", count="exact")
            .eq("activated", activated)
            .execute()
        )
    else:
        response = (
            supabase.table("sensor_registry").select("*", count="exact").execute()
        )
    return response
...
def getSensorByID(sensor_id):
    ...
    response, count = (
        supabase.table("sensor_registry").select("*").eq("id", sensor_id).execute()
    )
    if len(response[1]) == 0:
...
def getLatestData(sensor_id):
    ...
    response = (
        ...
        .select("data")
        .eq("sensor_name", response["sensor_name"])
        .execute()
    )
    print("🔥")
    print(response)
    ...
    return {
        "data_format": json.loads(data_info),
